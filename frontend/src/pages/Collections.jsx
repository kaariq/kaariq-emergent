      ...
      <section className="max-w-[1400px] mx-auto px-6 lg:px-10 mt-12">
        <div className="flex flex-wrap gap-2 mb-8">
          {sub.flatMap(c => c.items).map(it => {
            ...
            const active = s === slug;
            return (
              <Link key={it} to={`/collections/${s}`} className={`text-[11px] tracking-[0.22em] uppercase px-4 py-2 border transition-colors ${active ? 'bg-[hsl(351,33%,18%)] text-[hsl(30,22%,95%)] border-[hsl(351,33%,18%)]' : 'border-[hsl(28,11%,70%)] hover:border-[hsl(351,33%,18%)]'}`}>{it}</Link>
            );
          })}
        </div>
        <div className="grid lg:grid-cols-12 gap-6 lg:gap-8 mb-16">
          {COLLECTIONS.map((c, i) => (
            <Link to={`/collections/${c.tag.toLowerCase().replace(/\s+/g,'-')}`} key={c.title} className={`group relative overflow-hidden bg-[hsl(30,14%,82%)] block ${i%2===0 ? 'lg:col-span-7 aspect-[4/3]' : 'lg:col-span-5 aspect-[4/3]'}`}>
